...
#ifndef MY_ACE_DEV_POLL_REACTOR_H
#define MY_ACE_DEV_POLL_REACTOR_H
...
class My_ACE_Dev_Poll_Reactor;


// ---------------------------------------------------------------------

/**
* @class My_ACE_Dev_Poll_Reactor_Notify
*
* @brief Event handler used for unblocking the My_ACE_Dev_Poll_Reactor
*        from its event loop.
*
* This event handler is used internally by the My_ACE_Dev_Poll_Reactor
* as a means to allow a thread other then the one running the event
* loop to unblock the event loop.
*/
class My_ACE_Dev_Poll_Reactor_Notify : public ACE_Reactor_Notify
{
public:
	...
	*/
	...
	/// Called back by the My_ACE_Dev_Poll_Reactor when a thread wants to
	/// unblock us.
	...
	*/
	...
	/// Method called by My_ACE_Dev_Poll_Reactor to obtain one notification.
...
protected:
	...
	/**
	* Keep a back pointer to the My_ACE_Dev_Poll_Reactor.  If this value
	* if NULL then the My_ACE_Dev_Poll_Reactor has been initialized with
	* disable_notify_pipe.
	*/
	My_ACE_Dev_Poll_Reactor *dp_reactor_;

	/**
	* Contains the ACE_HANDLE the My_ACE_Dev_Poll_Reactor is listening
	* on, as well as the ACE_HANDLE that threads wanting the attention
	* of the My_ACE_Dev_Poll_Reactor will write to.
	*/
	ACE_Pipe notification_pipe_;

	/**
	* Keeps track of the maximum number of times that the
	* My_ACE_Dev_Poll_Reactor_Notify::handle_input method will iterate and
...
*
* The My_ACE_Dev_Poll_Reactor uses the `/dev/poll' or '/dev/epoll'
...
* handled at any given time.  As such, the My_ACE_Dev_Poll_Reactor can
...
*       resolutions with the @c My_ACE_Dev_Poll_Reactor.  However, the
...
class My_ACE_Dev_Poll_Reactor : public ACE_Reactor_Impl
{
...
public:
	...
	/// Initialize @c My_ACE_Dev_Poll_Reactor with the default size.
	/**
	* The default size for the @c My_ACE_Dev_Poll_Reactor is the maximum
	* number of open file descriptors for the process.
	*/
	My_ACE_Dev_Poll_Reactor (ACE_Sig_Handler * = 0,
		ACE_Timer_Queue * = 0,
	...
	/// Initialize My_ACE_Dev_Poll_Reactor with size @a size.
	/**
	...
	*       access violations.
	*/
	My_ACE_Dev_Poll_Reactor (size_t size,
		bool restart = false,
		ACE_Sig_Handler * = 0,
		ACE_Timer_Queue * = 0,
		int disable_notify_pipe = 0,
	...
	virtual ~My_ACE_Dev_Poll_Reactor (void);
	...
	*
	* @note It is only possible to achieve millisecond timeout
	*       resolutions with the @c My_ACE_Dev_Poll_Reactor.
	*/
	...
	*       My_ACE_Dev_Poll_Reactor.  Multiple threads may invoke the
...
protected:
	...
	*
	* @note It is only possible to achieve millisecond timeout
	*       resolutions with the My_ACE_Dev_Poll_Reactor.
	*/
...
/**
* @class My_ACE_Dev_Poll_Handler_Guard
*
...
public:
	...
	*/
	My_ACE_Dev_Poll_Handler_Guard (ACE_Event_Handler *eh, bool do_incr = true);
	...
	*/
	~My_ACE_Dev_Poll_Handler_Guard (void);
...
# include "My_Dev_Poll_Reactor.inl"
...
#endif  /* MY_ACE_DEV_POLL_REACTOR_H */
