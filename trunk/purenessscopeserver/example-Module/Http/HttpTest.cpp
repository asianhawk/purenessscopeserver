...
static const char *g_szDesc      = "Http模块";       //模块的描述文字
static const char *g_szName      = "Http测试";       //模块的名字
static const char *g_szModuleKey = "Http";           //模块的Key
...
int LoadModuleData(CServerObject* pServerObject)
{
	...
	{
		pMessageManager->AddClientCommand(COMMAND_HTTP_DATA, &g_BaseCommand, g_szName);
...
int UnLoadModuleData()
{
	...
	if(g_pServerObject != NULL)
	{
		...
		if(NULL != pMessageManager)
		{
			pMessageManager->DelClientCommand(COMMAND_HTTP_DATA, &g_BaseCommand);
