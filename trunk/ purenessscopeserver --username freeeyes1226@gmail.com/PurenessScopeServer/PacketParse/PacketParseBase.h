...
public:
	...
	virtual bool SetPacketHead(ACE_Message_Block* pmbHead, IMessageBlockManager* pMessageBlockManager) = 0;          //设置得到的数据包头，并在这里解密
	virtual bool SetPacketBody(ACE_Message_Block* pmbBody, IMessageBlockManager* pMessageBlockManager) = 0;          //设置得到的数据包体，并在这里解密 
	virtual uint8 GetPacketStream(ACE_Message_Block* pCurrMessage, IMessageBlockManager* pMessageBlockManager) = 0;  //专门对应 Mode为0的不带包头的数据包,如果是带包头的模式，这里什么都不用做。
	virtual bool MakePacket(const char* pData, uint32 u4Len, ACE_Message_Block* pMbData) = 0;                        //创建返回包体结构
	virtual uint32 MakePacketLength(uint32 u4DataLen)                                    = 0;                        //预先得到返回包体的长度
